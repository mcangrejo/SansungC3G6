...
int clave[3]={0},usuario[3]={0};  //estas son variables globales

void obtenerclave(void);  //prototipo de una funci[on. C lo exige. La presentacion en sociedad de la funcion
short int verificarfijas(void); //la funcion retorna un valor entero de tipo short int
void verificarpicas(short int*);  //la funcion recibe como parametro un apuntador(direccion) de una variable

int main()
{
   short int picas=0, fijas=0, intentos=0;	//estas son una variable local
   obtenerclave();	//paso 1
   while(fijas!=3)  //paso 2
   {
		...
		fijas=verificarfijas(); //paso 2c
        verificarpicas(&picas); //paso 2d. Como picas es local, se pasa la direccion de la variable como parametro para que la fucnion verificarpicas la pueda modificar
...
short int verificarfijas()
{
	char j;
	short int misfijas=0; //se debe inciializar fijas al llamar la funcion para evitar que se acumulen de los turnos anteriores 
	for(j=0;j<3;j++)
	{
	...
	return misfijas;  //se retorna las fijas calculadas
}

void verificarpicas(short int *picas) //recibe como parametro un apuntador a la variable local picas del main
{
	char u,c;
